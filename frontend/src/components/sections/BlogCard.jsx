...
export default function BlogCard({ card: initialCard, className, blockId, allCards, isEditing: sectionEditing, isAdmin, onUpdate, onDelete }) {
  ...
  const handleSave = async () => {
    try {
      ...
      // update server only if the containing block is already persisted
      if (blockId && !String(blockId).startsWith("new-")) {
        await ContentBlockService.updateBlock(blockId, { cards: updatedCards });
      }
      setLocalEditing(false);
      if (onUpdate) onUpdate(card); // inform parent about the updated data
    } catch (err) {
