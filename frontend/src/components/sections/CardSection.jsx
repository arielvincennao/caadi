...
export default function CardSection({ card: initialCard, className, onClick, isActive }) {
    ...
    const handleSave = (e) => {
        ...
        console.log("Guardando cambios de CardSection...", card);
    ...
    // Si estamos editando, forzamos que sea un 'div' para que los inputs no activen el botón/link
    ...
    return (
        ...
        >
                ...
                <div className="absolute top-2 right-2 z-10 flex gap-1">
                    {!isEditing ? (
                        ...
                        <div className="flex gap-1">
                            <button
                                onClick={handleSave}
                                className="bg-green-600 text-white px-2 py-1 rounded text-[10px] font-bold shadow-sm"
                            >
                                OK
                            </button>
                            <button
                                onClick={(e) => { e.stopPropagation(); setIsEditing(false); setCard(initialCard); }}
                                className="bg-gray-500 text-white px-2 py-1 rounded text-[10px] font-bold shadow-sm"
                            >
                                X
                            </button>
                        </div>
            ...
            {/* URL EDITABLE (Solo si ya era un link) */}
            {isEditing && card.href && (
