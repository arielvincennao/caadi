...
export default function MapBlock({ block, isEditing, isAdmin, onChange }) {
    ...
    const handleOfficeSelect = (e) => {
        ...
        const selectedOffice = offices.find(o => String(o.id) === String(selectedId)) || {};
    ...
    const handleSave = () => {
        // Confirmamos cambios al padre y cerramos la edición local
        onChange && onChange(block.id, data);
        setLocalEditing(false);
    };

    const handleCancel = () => {
        // Revertimos al estado original del bloque si el usuario cancela
        setData(block.data || {});
        setLocalEditing(false);
    };
    ...
    return (
        <div className={`mb-6 ${isAdmin && isEditing ? 'mt-6 flex' : ''}${localEditing ? 'flex-column' : ''}`}>
            ...
            {isAdmin && isEditing && (
                <div className="ms-2 z-10">
                    {!localEditing ? (
                        ...
                        <BtnControl onClick={() => { handleSave() }} title={"Dejar de editar bloque"} className={"p-2 bg-green-600 hover:bg-green-700 text-white cursor-pointer"}><Icon name={"check"} className={"w-5 h-5"} /></BtnControl>
                    )}
                </div>
            )}
        </div>
