...
export default function LinkForm({ initialData = {}, onSubmit, onCancel }) {
  ...
  const validate = () => {
    ...
    if (!form.href.trim()) newErrors.href = "La URL destino es obligatoria";
    return newErrors;
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="p-4 space-y-3">
      ...
      {errors.name && <p className="text-red-500 text-sm font-bold uppercase">{errors.name}</p>}

      <input
        name="name"
        value={form.name}
        onChange={handleChange}
        className={`w-full p-2 border rounded border-blue-600 ${errors.name ? "border-red-500" : ""
            }`}
      />
      ...
      <input
        ...
        className={`w-full p-2 border rounded border-blue-600 ${errors.href ? "border-red-500" : ""
            }`}
      />
